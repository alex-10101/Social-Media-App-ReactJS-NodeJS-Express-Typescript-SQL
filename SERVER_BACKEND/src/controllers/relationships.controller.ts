...
import {
  ...
  getRelationshipDB,
} from "../sqlQueries/relationships.sqlQuery";
...
export async function getRelationship(
  ...
  try {
    const followedUserId = sanitizeUserInput(req.query.followedUserId) as string;
...
export async function addAndDeleteRelationship(
  ...
  try {
    const userId = sanitizeUserInput(req.query.userId) as string;
